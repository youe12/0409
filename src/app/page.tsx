...
function QuestionGraphic(
    ...
    if (questionId === 4) {
        return (
            <div
                className="flex items-center justify-center gap-4 py-6 my-4 bg-indigo-50 rounded-xl">
...
function ShapeDisplay(
    ...
    if (shape === "circle") {
        return (
            <div
                className={`${sizeClass} rounded-full bg-indigo-500 border-2 border-indigo-600`} />
        );
    }
    ...
    if (shape === "square") {
        return (
            <div
                className={`${sizeClass} rounded-md bg-purple-500 border-2 border-purple-600`} />
        );
    }
...
const {
    assessmentInfo: staticInfo,
    questionBank: staticQuestions
} = questionsData;
...
export default function AssessmentPage() {
    ...
    const calculateDimensionScores = () => {
        ...
        return Object.entries(dimensionScores).map((
            [name, {
                total,
                count
            }]
    ...
    const currentAnswer = answers.find(a => a.questionId === questions[currentQuestionIndex]?.questionId);
    const progressPercent = questions.length > 0 ? Math.round(answers.length / questions.length * 100) : 0;
        ...
        return (
            <div
                ...
                <div className="text-center">
                    <div
                        className="w-16 h-16 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
        ...
        <div
            ...
            <header className="bg-white shadow-sm sticky top-0 z-50">
                <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
                    ...
                    {status === "in_progress" && <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <main className="max-w-3xl mx-auto px-4 py-8">
                {}
                {status === "intro" && <div className="animate-fade-in">
                    {}
                    <div className="text-center mb-12">
                        <div
                            ...
                            <Star className="w-4 h-4" />专业儿童天赋测评
                                          </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-6 mb-12">
                        ...
                        <Card className="bg-white border-0 shadow-lg">
                            <CardContent className="p-6 text-center">
                                <div
                                    className="w-14 h-14 bg-purple-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    ...
                    <Card className="bg-white border-0 shadow-lg mb-8">
                        <CardContent className="p-6">
                            <h2 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                                <CheckCircle2 className="w-5 h-5 text-green-500" />测评须知
                                                </h2>
                    ...
                    <div className="text-center">
                        <Button
                            ...
                            className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white px-12 py-6 text-lg font-medium rounded-2xl shadow-lg hover:shadow-xl transition-all">开始测评
                                            <ChevronRight className="w-5 h-5 ml-2" />
                        </Button>
                    </div>
                </div>}
                {}
                {status === "child_info" && <div className="animate-fade-in">
                    <Card className="bg-white border-0 shadow-lg max-w-md mx-auto">
                        <CardContent className="p-8">
                            <h2 className="text-2xl font-bold text-gray-900 mb-2 text-center">请填写孩子信息
                                                </h2>
                            <p className="text-gray-600 text-center mb-8">帮助我们提供更准确的分析
                                                </p>
                            <div className="space-y-6">
                                <div>
                                    <Label htmlFor="name" className="text-gray-700 font-medium">孩子姓名（选填）
                                                            </Label>
                                    <Input
                                        id="name"
                                        placeholder="请输入姓名"
                                ...
                                <div>
                                    <Label className="text-gray-700 font-medium">孩子年龄 <span className="text-red-500">*</span>
                                    </Label>
                                    <div className="flex gap-3 mt-2">
                                        {[6, 7, 8, 9, 10, 11, 12].map(age => <button
                                            key={age}
                                            onClick={() => setChildInfo({
                                                ...childInfo,
                                                age
                                            })}
                                            className={`w-12 h-12 rounded-xl font-medium transition-all ${childInfo.age === age ? "bg-indigo-500 text-white shadow-lg" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
                                            {age}
                                        </button>)}
                                    </div>
                                </div>
                                <Button
                                    ...
                                    className="w-full h-12 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-medium rounded-xl">开始答题
                                                        <ChevronRight className="w-5 h-5 ml-2" />
                                </Button>
                            </div>
                        </CardContent>
                ...
                {}
                {status === "in_progress" && currentQuestion && <div className="animate-fade-in">
                    {}
                    <div className="mb-6">
                    ...
                    <Card className="question-card mb-6">
                        <CardContent className="p-6 md:p-8">
                            ...
                            <div className="flex items-center gap-2 mb-4">
                                <span
                                    className={`px-3 py-1 rounded-full text-xs font-medium ${currentQuestion.assessmentType === "韦氏智力测评" ? "bg-blue-100 text-blue-700" : currentQuestion.assessmentType === "多元智能测试" ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-700"}`}>
                            ...
                            {}
                            <h2
                                className="text-xl md:text-2xl font-semibold text-gray-900 mb-2 leading-relaxed">
                                观察下面图形排列，下一个图形应该是？
                            </h2>
                            {}
                            ...
                            <RadioGroup
                                ...
                                className="space-y-3">
                                {currentQuestion.options.map(opt => <div key={opt.option} className="relative">
                                    <RadioGroupItem value={opt.option} id={`option-${opt.option}`} className="peer sr-only" />
                                    <Label
                                        htmlFor={`option-${opt.option}`}
                                        className={`option-btn flex items-center p-4 rounded-xl cursor-pointer ${currentAnswer?.selectedOption === opt.option ? "selected" : ""}`}>
                                        <span
                                            className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold mr-4 ${currentAnswer?.selectedOption === opt.option ? "bg-white/20 text-white" : "bg-indigo-100 text-indigo-700"}`}>
                                            {opt.option}
                                        </span>
                                        <span
                                            className={currentAnswer?.selectedOption === opt.option ? "text-white" : ""}>
                                            {opt.content}
                                        </span>
                                    </Label>
                                </div>)}
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        {currentQuestionIndex === questions.length - 1 ? <Button
                            ...
                            className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white px-8">提交测评
                                              <CheckCircle2 className="w-4 h-4 ml-2" />
                        </Button> : <Button
                            onClick={handleNext}
                            className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white px-6">下一题
                    ...
                    <div className="mt-8 flex justify-center gap-1 flex-wrap">
                        {questions.map((q, idx) => {
                            ...
                            return (
                                <div
                                    key={q.questionId}
                                    className={`w-3 h-3 rounded-full transition-all ${isCurrent ? "bg-indigo-500 scale-125" : isAnswered ? "bg-indigo-300" : "bg-gray-200"}`} />
                            );
                        })}
                    </div>
                </div>}
                {}
                {status === "analyzing" && <div className="animate-fade-in text-center py-12">
                    <div
                        className="w-20 h-20 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mx-auto mb-6"></div>
                ...
                {}
                {status === "report" && <div className="animate-fade-in">
                    {}
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-600">基于韦氏智力测评框架 + 多元智能理论 | 共 {questions.length}题
                                          </p>
                    </div>
                    {}
                    ...
                    <Card className="bg-white border-0 shadow-lg mb-8">
                        <CardContent className="p-6">
                            ...
                            <div className="space-y-4">
                                {calculateDimensionScores().sort((a, b) => b.score - a.score).map(dim => <ScoreProgress key={dim.name} name={dim.name} score={dim.score} />)}
                            </div>
                        </CardContent>
                    </Card>
                    {}
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Button onClick={() => window.print()} variant="outline" className="px-8">打印报告
                                          </Button>
                        <Button
                            onClick={handleRestart}
                            className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white px-8">重新测评
                                          </Button>
                    </div>
                </div>}
            </main>
...
function ReportRenderer(
    ...
    const flushSection = () => {
        if (currentSection.length > 0) {
            ...
            if (text) {
                elements.push(<div
                    key={sectionKey++}
                    className="mb-4 whitespace-pre-wrap text-gray-700 leading-relaxed">
                    {text}
                </div>);
            }
    ...
    lines.forEach(line => {
        ...
        if (trimmed.startsWith("## ")) {
            ...
            elements.push(<h2
                key={sectionKey++}
                className="text-xl font-bold text-gray-900 mt-8 mb-4 flex items-center gap-2">
            ...
            elements.push(<h3
                key={sectionKey++}
                className="text-lg font-semibold text-indigo-700 mt-6 mb-3">
                {trimmed.replace("### ", "")}
            </h3>);
        } else if (trimmed.startsWith("**") && trimmed.endsWith("**")) {
            flushSection();

            elements.push(<p key={sectionKey++} className="font-bold text-gray-900 mt-4 mb-2">
                {trimmed.replace(/\*\*/g, "")}
            </p>);
        } else if (/^\d+\./.test(trimmed)) {
            flushSection();

            elements.push(<div key={sectionKey++} className="flex gap-3 mb-2">
                <span
                    className="w-6 h-6 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center text-sm font-medium flex-shrink-0">
                    {trimmed.charAt(0)}
                </span>
